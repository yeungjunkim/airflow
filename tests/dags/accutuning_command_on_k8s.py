...
command_worker = KubernetesPodExOperator(
    ...
    env_vars={
        "ACCUTUNING_WORKSPACE": "{{ti.xcom_pull(key=ACCUTUNING_WORKSPACE, task_id='make_parameters') }}",
        "ACCUTUNING_LOG_LEVEL": "{{ti.xcom_pull(key=ACCUTUNING_LOG_LEVEL, task_id='make_parameters') }}",
        "ACCUTUNING_USE_LABELER": "{{ti.xcom_pull(key=ACCUTUNING_USE_LABELER, task_id='make_parameters') }}",
        "ACCUTUNING_USE_CLUSTERING": "{{ti.xcom_pull(key=ACCUTUNING_USE_CLUSTERING, task_id='make_parameters') }}",
        "DJANGO_SETTINGS_MODULE": "{{ti.xcom_pull(key=DJANGO_SETTINGS_MODULE, task_id='make_parameters') }}",
        "ACCUTUNING_DB_ENGINE": "{{ti.xcom_pull(key=ACCUTUNING_DB_ENGINE, task_id='make_parameters') }}",
        "ACCUTUNING_DB_HOST": "{{ti.xcom_pull(key=ACCUTUNING_DB_HOST, task_id='make_parameters') }}",
        "ACCUTUNING_DB_PORT": "{{ti.xcom_pull(key=ACCUTUNING_DB_PORT, task_id='make_parameters') }}",
        "ACCUTUNING_DB_NAME": "{{ti.xcom_pull(key=ACCUTUNING_DB_NAME, task_id='make_parameters') }}",
        "ACCUTUNING_DB_USER": "{{ti.xcom_pull(key=ACCUTUNING_DB_USER, task_id='make_parameters') }}",
        "ACCUTUNING_DB_PASSWORD": "{{ti.xcom_pull(key=ACCUTUNING_DB_PASSWORD, task_id='make_parameters') }}",
    },
    cmds=["python3"],
