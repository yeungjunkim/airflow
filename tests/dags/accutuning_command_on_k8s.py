...
command_worker = KubernetesPodExOperator(
    ...
    env_vars={
        "ACCUTUNING_WORKSPACE": "{{ti.xcom_pull(key=ACCUTUNING_WORKSPACE) }}",
        "ACCUTUNING_LOG_LEVEL": "{{ti.xcom_pull(key=ACCUTUNING_LOG_LEVEL) }}",
        "ACCUTUNING_USE_LABELER": "{{ti.xcom_pull(key=ACCUTUNING_USE_LABELER) }}",
        "ACCUTUNING_USE_CLUSTERING": "{{ti.xcom_pull(key=ACCUTUNING_USE_CLUSTERING) }}",
        "DJANGO_SETTINGS_MODULE": "{{ti.xcom_pull(key=DJANGO_SETTINGS_MODULE) }}",
        "ACCUTUNING_DB_ENGINE": "{{ti.xcom_pull(key=ACCUTUNING_DB_ENGINE) }}",
        "ACCUTUNING_DB_HOST": "{{ti.xcom_pull(key=ACCUTUNING_DB_HOST) }}",
        "ACCUTUNING_DB_PORT": "{{ti.xcom_pull(key=ACCUTUNING_DB_PORT) }}",
        "ACCUTUNING_DB_NAME": "{{ti.xcom_pull(key=ACCUTUNING_DB_NAME) }}",
        "ACCUTUNING_DB_USER": "{{ti.xcom_pull(key=ACCUTUNING_DB_USER) }}",
        "ACCUTUNING_DB_PASSWORD": "{{ti.xcom_pull(key=ACCUTUNING_DB_PASSWORD) }}",
    },
    cmds=["python3"],
